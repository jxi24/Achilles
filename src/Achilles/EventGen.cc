...
double achilles::EventGen::GenerateEvent(const std::vector<FourVector> &mom, const double &wgt) {
    ...
    if(outputEvents) {
        ...
        if(outputCurrentEvent) {
            ...
            fmt::print("Beam Energy = {:^8.5e}\n", config["Beams"][0]["Beam"]["Beam Params"]["Energy"].as<double>()); */
            bool anti_b = ((scattering -> Process().m_ids)[0].AsInt() < 0);
            double anti = 1;
            if (anti_b) {
                anti = 0;
            }
            try {
                ...
                if (data.is_open() && Amps2[anti] == Amps2[anti]) {
                    data << Q0 << "\t" << Theta_degrees << "\t" << event.Weight() << "\t" << event.get_p_num_l()[anti] * 6 * wgt << "\t" 
                    << event.get_p_num_t()[anti] * 6 * wgt << "\n";
                }
                else if (Amps2[anti] != Amps2[anti]) {
